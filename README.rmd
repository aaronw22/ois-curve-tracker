...
# Graph the change in the implied cash rate between the two most recent periods

all_data[, change := cash_rate - shift(cash_rate), by = date]

ggplot(all_data[scrape_date == max(scrape_date)], aes(x = date, y = change)) +
  geom_col(fill = "#0b5394") +
  labs(title = "Change in Implied Cash Rate from Previous Day",
