...
dates_to_keep[, days_before := as.numeric(max(scrape_date) - scrape_date)]
...
# Removing this graph now that there are no more cash rate cuts priced in (Dec 25)
...
ggplot(graph_data, aes(x = scrape_date, y = value, colour = variable)) +
  ...
  labs(title = "Cash rate change by period ahead",
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(trans = ~ . / 0.25, name = "Number of 0.25ppt rises")
    ) +
  scale_colour_viridis_d() +
  theme_bw() + 
...
```{r graph-total-cuts-3m, echo=FALSE, dev='svg', warning=FALSE}
...
# Show past year only
graph_data <- graph_data[scrape_date > Sys.Date() %m-% months(3)]
...
ggplot(graph_data, aes(x = scrape_date, y = value, colour = variable)) +
  ...
  geom_text_repel(data = label_data, aes(label = paste0(variable, ": ", round(value, 2), "%")),
                  nudge_x = 7, segment.colour = NA, hjust = 0,
                  xlim = c(latest_date, NA)) +
  labs(title = "Cash rate change by period ahead (3 month history)",
       x = "Data scrape date", y = "Change in cash rate (ppt)") +
  scale_x_date(date_labels = "%b %Y", date_breaks = "1 months", limits = expand_date_axis) +
  scale_y_continuous(
    breaks = function(x) seq(rounder(range(x)[1], 0.25), rounder(range(x)[2], 0.25), by = 0.25),
    sec.axis = sec_axis(trans = ~ . / 0.25, name = "Number of 0.25ppt rises")
    ) +
